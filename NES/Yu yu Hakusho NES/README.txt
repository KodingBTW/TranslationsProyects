Yu Yu Hakusho - Bakutou Ankoku Bujutsukai (J).nes
Traducido por koda 2024

-------------------------------------------------------------------------------------------
ÍNDICE
-------------------------------------------------------------------------------------------

1. INSTRUCCIONES
2. VERSIONES
3. ERRORES CONOCIDOS
4. INFORMACIÓN ADICIONAL
5. AGRADECIMIENTOS
6. CONTACTO

--------------------------------------------------------------------------------------------
1. INSTRUCCIONES
--------------------------------------------------------------------------------------------

La ROM es:

Database match: Datach - Yu Yu Hakusho - Bakutou Ankoku Bujutsukai (Japan)
Database: No-Intro: Nintendo Entertainment System (v. 20210216-231042)
File SHA-1: 4430F96F46EEE3BE92EA7F6ECCEE0076118189D1
File CRC32: 44070CFA
ROM SHA-1: 74218AAE93E4FEBFB2284BCF15811453418A2029
ROM CRC32: F51A7F46

Hay 2 archivos el normal, y otro con el "barcode mode", permite facilmente elegir a los
jugadores, ya que el juego opera con Datach.

Utilizen "Lunar IPS" o "Floating IPS"
Pueden descargarlo de aquí:
https://www.romhacking.net/utilities/240/

--------------------------------------------------------------------------------------------
2. VERSIONES
--------------------------------------------------------------------------------------------

versión 1.0 (13/12/2024):

- 99% traducido, excepto game over screen, el title screen tampoco.
- Editados punteros.
- Agregados caracteres del español.

--------------------------------------------------------------------------------------------
3. ERRORES CONOCIDOS
--------------------------------------------------------------------------------------------

- Ninguno que yo sepa, informar alguno.

--------------------------------------------------------------------------------------------
4. INFORMACIÓN ADICIONAL
--------------------------------------------------------------------------------------------

La imagenes estaban comprimidas con una rutina RLE de 3 bytes,
https://github.com/KodingBTW/yuyuhakushobabnes-graphicsextractor

--------------------------------------------------------------------------------------------
5. AGRADECIMIENTOS
--------------------------------------------------------------------------------------------

-  A todo el equipo de Stardust Crusaders, por hacer la traducción al inglés de la cual me 
he basado https://www.romhacking.net/translations/5386/

- Al parche de wangsitan que remueve la obligatoriedad del Datach.
	https://www.romhacking.net/hacks/1979/

- A bunkai por las guias de descompresión de gráficos.

--------------------------------------------------------------------------------------------
6. CONTACTO
--------------------------------------------------------------------------------------------

Traducción hecha por rodrigo muñoz, alias koda. Visita https://traduccioneskoda.blogspot.com 
para más traducciones.

Correo contacto: traduccioneskoda@gmail.com